...
import { 
  ...
  Download, FileUp, Edit2, Search, Printer, Hammer, Calendar, User, FileDigit, ChevronRight, FileSpreadsheet
} from 'lucide-react';
import { 
...
// Clave única v9 para evitar conflictos
const K = {
  STABLE: 'mb_data_v9'
};
...
const App: React.FC = () => {
  ...
  // PERSISTENCIA BLINDADA (AUTO-SAVE)
  useEffect(() => {
    if (loading) return;
    const data = { products, mps, recipes, orders, history, user };
    localStorage.setItem(K.STABLE, JSON.stringify(data));
  ...
  const loadData = useCallback(async () => {
    ...
    if (saved) {
      ...
      setProducts(INITIAL_CATALOG.map((p, i) => ({ ...p, id: `p-${i}`, wip: 0 })));
    }

    // 2. Si hay internet, intentar sincronizar PERO NO SOBRESCRIBIR si la nube está vacía
    if (isOnline) {
      try {
        ...
        if (mRes.data && mRes.data.length > 0) setMps(mRes.data);
        if (rRes.data && rRes.data.length > 0) setRecipes(rRes.data);
        if (oRes.data && oRes.data.length > 0) setOrders(oRes.data);
        if (hRes.data && hRes.data.length > 0) setHistory(hRes.data);
    ...
    printRoot.innerHTML = `
      <div style="padding: 20px;">
        ...
        <table style="width:100%; border-collapse:collapse; margin-top:10px;">
          <thead><tr style="background:#eee;"><th>SKU</th><th>MARCA/MODELO</th><th>MIN</th><th>STOCK</th><th>TALLER</th></tr></thead>
          <tbody>${products.map(p => `<tr style="${p.stock < p.min ? 'color:red; font-weight:bold;' : ''}"><td>${p.sku}</td><td>${p.marca} ${p.modelo}</td><td>${p.min}</td><td>${p.stock}</td><td>${p.wip}</td></tr>`).join('')}</tbody>
        </table>

        <h2 style="margin-top:30px; background:#f1f5f9; padding:8px;">2. STOCK DE MATERIAS PRIMAS / INSUMOS</h2>
        <table style="width:100%; border-collapse:collapse; margin-top:10px;">
          <thead><tr style="background:#eee;"><th>CÓDIGO</th><th>DESCRIPCIÓN</th><th>MIN</th><th>STOCK</th></tr></thead>
          <tbody>${mps.map(m => `<tr style="${m.stock < m.min ? 'color:red; font-weight:bold;' : ''}"><td>${m.sku}</td><td>${m.desc}</td><td>${m.min}</td><td>${m.stock}</td></tr>`).join('')}</tbody>
        </table>
      </div>
    `;
    window.print();
      ...
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
        <Card className="w-full max-w-sm p-10 bg-white rounded-[3rem] shadow-2xl">
          ...
          <form onSubmit={(e) => { e.preventDefault(); if (tempName.trim()) setUser({ name: tempName, role: 'admin' }); }} className="space-y-4">
            ...
            <Button type="submit" className="w-full py-4 rounded-2xl" icon={ArrowRight}>Entrar</Button>
          </form>
        </Card>
      </div>
    );
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F8FAFC]">
      <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-8 shadow-sm no-print">
        ...
        <div className="flex items-center gap-4">
          <Badge type={isOnline ? 'ok' : 'warn'} text={isOnline ? 'CONECTADO' : 'SIN RED'} />
        ...
        <main className="flex-1 overflow-y-auto p-6 lg:p-10 custom-scrollbar">
          {activeTab === ViewType.DASHBOARD && (
            <div className="space-y-8">
              <SectionHeader title="Visión General" subtitle={`Bienvenido, ${user.name}`} />
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                <StatCard title="Stock Espejos" value={products.reduce((a, b) => a + b.stock, 0)} icon={Package} color="blue" />
                <StatCard title="En Taller" value={products.reduce((a, b) => a + (b.wip || 0), 0) + mps.reduce((a, b) => a + (b.wip || 0), 0)} icon={Hammer} color="blue" />
                <StatCard title="Total Insumos" value={mps.reduce((a, b) => a + b.stock, 0)} icon={Layers} color="blue" />
              ...
              <Card className="p-8 bg-indigo-50 border-indigo-200">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="font-black text-indigo-900 flex items-center gap-3"><BrainCircuit size={24} /> Asistente de Planta</h3>
                  <Button onClick={() => { setIsAiLoading(true); getAIInventoryAdvice(products, mps).then(setAiAdvice).finally(()=>setIsAiLoading(false)) }} disabled={isAiLoading} variant="special">
                    {isAiLoading ? 'Pensando...' : 'Pedir Consejo'}
                  </Button>
                </div>
                {aiAdvice.length > 0 && (
          ...
          {activeTab === ViewType.RAW_MATERIALS && (
            <div className="space-y-8">
              <SectionHeader title="Materias Primas" action={
                <div className="flex gap-2">
                  <Button onClick={handlePrint} variant="secondary" icon={Printer}>Reporte</Button>
              ...
              <div className="relative max-w-sm"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><Input placeholder="Buscar insumos..." className="pl-12 py-3" value={searchMpQuery} onChange={e => setSearchMpQuery(e.target.value)} /></div>
              {showAddMp && (
                <Card className="p-8 bg-emerald-50 border-emerald-200 animate-in slide-in-from-top-2">
                  <div className="flex justify-between items-center mb-6"><h3 className="font-bold">Nuevo Insumo / Semielaborado</h3><button onClick={()=>setShowAddMp(false)}><X/></button></div>
                  <form onSubmit={(e) => {
                    e.preventDefault();
                    ...
                    <Input name="min" type="number" defaultValue="10" />
                    <Input name="stock" type="number" defaultValue="0" />
                    <Button type="submit" variant="success" className="col-span-full py-4">Guardar Insumo</Button>
                  </form>
                </Card>
              )}
          ...
          {activeTab === ViewType.PRODUCTS && (
            <div className="space-y-8">
               <SectionHeader title="Stock Real (Espejos)" action={
                 <div className="flex gap-2">
                   <Button onClick={handlePrint} variant="secondary" icon={Printer}>Reporte</Button>
                   <Button onClick={exportToCsv} variant="secondary" icon={FileSpreadsheet}>Excel</Button>
                   <label className="flex items-center gap-2 cursor-pointer bg-slate-100 px-4 py-2 rounded-xl hover:bg-slate-200 transition-all font-bold text-xs">
                   ...
                   <Button onClick={() => setShowAddProduct(true)} icon={Plus}>Añadir Espejo</Button>
                 </div>
               } />
               <div className="relative max-w-sm"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><Input placeholder="Buscar SKU..." className="pl-12 py-3" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} /></div>
              {showAddProduct && (
                 <Card className="p-8 bg-blue-50 border-blue-200">
                  ...
                  <form onSubmit={(e) => {
                    ...
                    <Button type="submit" className="col-span-full py-4">Guardar Espejo</Button>
                  </form>
                </Card>
              )}
          ...
          {activeTab === ViewType.REPORTS && (
            <div className="max-w-2xl mx-auto py-20">
              <Card className="p-12 text-center space-y-8 bg-white shadow-2xl rounded-[3rem]">
                ...
                <h3 className="text-3xl font-black text-slate-800">{isOnline ? 'Nube Conectada' : 'Modo Offline'}</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <Button variant="secondary" onClick={() => {
                    const blob = new Blob([localStorage.getItem(K.STABLE) || ''], { type: 'application/json' });
                    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `backup_${Date.now()}.json`; a.click();
                  }} className="col-span-full py-6" icon={Download}>Respaldar Todo</Button>
                </div>
              </Card>
            </div>
          )}
          ...
          {activeTab === ViewType.HISTORY && (
            <div className="space-y-6">
              <SectionHeader title="Historial de Auditoría" />
              <Card className="overflow-hidden shadow-lg">
                <table className="w-full text-left">
                  <thead className="bg-slate-800 text-white text-[10px] uppercase font-black">
                    <tr><th className="p-5">TIMESTAMP</th><th className="p-5">TIPO</th><th className="p-5">DETALLE</th><th className="p-5 text-right">OPERARIO</th></tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {history.map(h => (
                      <tr key={h.id} className="text-sm hover:bg-slate-50 transition-colors">
                        ...
                        <td className="p-5 italic text-slate-600">{h.detalle}</td>
      ...
      <table className="w-full text-left">
        <thead className="bg-slate-50 text-[10px] font-black uppercase text-slate-400">
          <tr>
            ...
            <th className="p-5">{type === 'products' ? 'MARCA / MODELO' : 'DESCRIPCIÓN'}</th>
            <th className="p-5 text-center">MÍN</th>
            <th className="p-5 text-center">STOCK</th>
            <th className="p-5 text-center">TALLER</th>
            <th className="p-5 text-right no-print">GESTIÓN</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-100">
          {data.map((item: any) => {
              ...
              <React.Fragment key={item.id}>
                <tr className={`hover:bg-slate-50 transition-all text-sm ${isEditing ? 'bg-blue-50/30' : ''}`}>
                  ...
                  <td className="p-5 text-right flex justify-end gap-2 no-print">
                    <button onClick={() => { setActiveActionId(isActionActive ? null : item.id); setQtyInput(1); setCliente(''); }} className={`p-2 rounded-xl transition-all ${isActionActive ? 'bg-emerald-600 text-white shadow-md' : 'text-emerald-600 hover:bg-emerald-50'}`}><Truck size={18}/></button>
                    <button onClick={() => setEditingId(isEditing ? null : item.id)} className={`p-2 rounded-xl transition-all ${isEditing ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-100'}`}>{isEditing ? <CheckCircle size={18}/> : <Edit2 size={18}/>}</button>
                    <button onClick={() => onDelete(item.id)} className="p-2 text-red-300 hover:text-red-600 hover:bg-red-50 rounded-xl"><Trash2 size={18}/></button>
                  </td>
                </tr>
                {isActionActive && (
                  <tr className="bg-slate-50/80 animate-in slide-in-from-top-1 border-b">
                    <td colSpan={6} className="p-6">
                      <div className="flex flex-wrap items-end gap-6">
                        ...
                        {type === 'products' && <div className="flex-1 min-w-[200px]"><label className="text-[10px] font-black uppercase text-slate-400 block mb-2">Cliente / Remito</label><Input placeholder="Ej: Transporte Merlobus" value={cliente} onChange={e => setCliente(e.target.value)} /></div>}
                        <div className="flex gap-2">
                          <Button variant="success" className="h-12 px-8 font-black" onClick={() => { onAction(item.id, qtyInput, type === 'products' ? { cliente } : undefined); setActiveActionId(null); }}>Confirmar</Button>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 animate-in fade-in">
      <Card className="p-8 h-fit space-y-6 shadow-xl">
        <h3 className="font-black text-xl">Composición Técnica</h3>
        <div className="flex gap-2 p-1 bg-slate-100 rounded-2xl">
          <button onClick={() => {setTargetType('product'); setSelId('');}} className={`flex-1 p-3 rounded-xl text-[10px] font-black uppercase transition-all ${targetType === 'product' ? 'bg-[#2B3860] text-white shadow-md' : 'text-slate-400'}`}>ESPEJOS</button>
          <button onClick={() => {setTargetType('mp'); setSelId('');}} className={`flex-1 p-3 rounded-xl text-[10px] font-black uppercase transition-all ${targetType === 'mp' ? 'bg-[#2B3860] text-white shadow-md' : 'text-slate-400'}`}>ESTRUCTURAS</button>
        </div>
      ...
      <Card className="md:col-span-2 p-10 min-h-[500px] shadow-2xl relative bg-white">
        <div className="flex justify-between items-center mb-8">
          <h3 className="font-black text-2xl text-slate-800">Insumos del Escandallo</h3>
          <Button variant="secondary" size="lg" icon={Plus} disabled={!selId} onClick={() => setShowAdd(!showAdd)}>{showAdd ? 'Cancelar' : 'Añadir Insumo'}</Button>
        </div>
        {showAdd && (
          <div className="bg-blue-50 p-6 rounded-3xl mb-8 grid grid-cols-3 gap-4 border-2 border-blue-100 animate-in slide-in-from-top-4">
            ...
            <Button className="col-span-full py-5 rounded-2xl text-lg font-black" variant="success" onClick={() => { if(selMpId && qty > 0) { onAddRecipeItem(selId, targetType, selMpId, qty); setShowAdd(false); setSelMpId(''); setQty(1); } }}>VINCULAR A RECETA</Button>
          </div>
        )}
          ...
          {currentRecipes.map((r:any) => (
            <div key={r.id} className="p-5 border-2 rounded-2xl flex justify-between items-center bg-white hover:border-blue-300 transition-all shadow-sm">
              ...
              <div className="flex items-center gap-8">
                ...
                <button onClick={() => onDeleteRecipeItem(r.id)} className="text-slate-300 hover:text-red-500 transition-colors"><Trash2 size={24}/></button>
              </div>
            </div>
          ))}
          {!selId && <div className="h-full flex flex-col items-center justify-center text-slate-200 py-20"><Settings size={80} className="opacity-10 mb-6" /><p className="text-xl">Elija un item a la izquierda para cargar su receta</p></div>}
        </div>
      </Card>
    </div>
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <Card className="p-10 space-y-6 shadow-2xl border-t-[10px] border-[#2B3860] bg-white rounded-[3rem]">
          <div className="flex justify-between items-center">
            <h3 className="font-black text-2xl uppercase tracking-tighter">Lanzar Lote</h3>
            <div className="flex bg-slate-100 p-1.5 rounded-2xl">
              <button onClick={()=>setMode('product')} className={`px-5 py-2 text-[10px] font-black rounded-xl transition-all ${mode==='product' ? 'bg-white shadow-md text-[#2B3860]':'text-slate-400'}`}>ESPEJOS</button>
              <button onClick={()=>setMode('mp')} className={`px-5 py-2 text-[10px] font-black rounded-xl transition-all ${mode==='mp' ? 'bg-white shadow-md text-[#2B3860]':'text-slate-400'}`}>ESTRUCTURAS</button>
            </div>
          </div>
          ...
          {selId && (
            <div className="bg-slate-50 p-6 rounded-3xl space-y-3 border shadow-inner">
               <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-2">Chequeo de Stock</p>
          ...
          <Button className="w-full py-8 text-2xl font-black rounded-[2rem] shadow-xl" variant="special" icon={Factory} disabled={!selId} onClick={()=>{onStartOrder(selId, mode, qty); setSelId(''); setQty(1);}}>EMPEZAR FABRICACIÓN</Button>
        </Card>

        {/* PANEL 2: INGRESO DE MATERIA PRIMA (RESTAURADO) */}
        <Card className="p-10 space-y-6 shadow-2xl border-t-[10px] border-emerald-600 bg-white rounded-[3rem]">
          <h3 className="font-black text-2xl uppercase tracking-tighter flex items-center gap-3"><Truck className="text-emerald-600"/> Entrada de Insumos</h3>
        ...
        <Card className="p-12 shadow-2xl bg-[#0f172a] text-white lg:col-span-2 rounded-[3.5rem] border border-slate-800">
          <div className="flex justify-between items-center mb-10 border-b border-slate-800 pb-6">
            <h3 className="font-black text-3xl uppercase tracking-widest flex items-center gap-4"><Hammer className="text-blue-400" /> Línea de Taller</h3>
            <Badge type="process" text={`${orders.length} ÓRDENES`} />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {orders.map((o:any) => (
              <div key={o.id} className="p-8 bg-slate-800/60 border-2 border-slate-700 rounded-[2.5rem] space-y-6 transition-all hover:border-blue-500 shadow-xl group">
                <div className="w-full">
                  <Badge type={o.targetType === 'mp' ? 'warn' : 'info'} text={o.targetType === 'mp' ? 'SEMI' : 'FINAL'} />
                  <p className="font-black text-white text-2xl mt-4 leading-tight group-hover:text-blue-300 transition-colors">{o.productName}</p>
                  <p className="text-blue-400 font-black text-lg uppercase mt-2">CANT: {o.qty}</p>
                </div>
                <Button onClick={() => onCompleteOrder(o)} variant="success" size="lg" className="w-full py-5 text-xl font-black rounded-3xl group-hover:scale-105 transition-transform">COMPLETAR</Button>
              </div>
            ))}
            {orders.length === 0 && <div className="col-span-full text-center py-20 text-slate-700 italic opacity-50 border-4 border-dashed border-slate-800 rounded-[3rem] text-2xl font-black uppercase">Planta en Espera</div>}
          </div>
        </Card>
      </div>
    </div>
...
const PlanningView = ({ products, mps, recipes }: any) => {
  ...
  // EXPLOSIÓN MULTINIVEL RECURSIVA - VERSIÓN 9
  const getFullRequirements = useCallback((targetId: string, targetType: 'product' | 'mp', multiplier: number, depth: number = 0): any[] => {
    ...
    immediate.forEach((r: any) => {
      ...
      // Si es un semielaborado, explotar sus componentes internos
  ...
  return (
    <div className="space-y-10 animate-in slide-in-from-bottom-2">
      <SectionHeader title="Planificación de Lote" subtitle="Explosión multinivel de materiales y semielaborados" />
      <Card className="p-10 bg-[#2B3860] text-white flex flex-col md:flex-row gap-8 items-end shadow-2xl rounded-[3rem]">
        <div className="flex-1 w-full"><label className="text-[10px] font-black uppercase text-blue-300 mb-4 block tracking-widest">Espejo Final</label>
      ...
      {sel ? (
        <Card className="p-10 bg-white rounded-[3rem] shadow-2xl">
          <h3 className="font-black text-2xl mb-10 border-b-4 border-slate-50 pb-6">Lista Maestra de Necesidades ({qty}u)</h3>
          <table className="w-full">
            <thead className="text-[10px] uppercase font-black text-slate-400">
              <tr className="border-b-2 border-slate-100">
                <th className="pb-5 text-left">DESGLOSE DE MATERIALES / SEMIE</th>
                  ...
                  <tr key={i} className={`text-sm hover:bg-slate-50 transition-colors ${req.depth > 0 ? 'bg-blue-50/10' : ''}`}>
                    <td className="py-6">
                      <div className="flex items-center gap-4" style={{ paddingLeft: `${req.depth * 40}px` }}>
                        ...
                        {isSemie && <Badge type="warn" text="SEMI-ELAB" />}
                      </div>
                    </td>
                    ...
                    <td className="py-6 text-right">
                      {diff >= 0 ? <Badge type="ok" text="DISPONIBLE" /> : <Badge type="danger" text={`REPONER ${Math.abs(diff)}`} />}
                    </td>
                  </tr>
                );
              })}
      ...
      ) : <div className="py-40 text-center text-slate-300 font-black uppercase text-2xl italic opacity-20 border-4 border-dashed border-slate-100 rounded-[3rem]">Seleccione un modelo para calcular</div>}
    </div>
  );
};
